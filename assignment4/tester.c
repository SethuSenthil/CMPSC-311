...
int test_cache_mru_insert();
int test_cache_mru_lookup();
int test_cache_resize();
...
int main(int argc, char *argv[])
{
  ...
  score += test_cache_mru_insert();
  score += test_cache_mru_lookup();
  score += test_cache_resize();
...
/* Testing mru in the case of insert-only workload. We insert 4 entries to a
 * cache of size 3. The last inserted entry should evict the first inserted
 * entry because all 3 entries have no of accesses as 1, and the 1st one that
 * was inserted in the cache would be evicted. */
int test_cache_mru_insert() {
  printf("running %s: ", __func__);
  ...
  if (cache_lookup(1, 7, out) != -1) {
    printf("failed: the fourth insert into a cache of 3 entries should evict the latest insert but it did not.\n");
    goto out;
  }
...
/* Testing mru in the case of a workload with inserts and lookups. We insert 3
 * entries to a cache of size 3. We then perform a lookup on some of them,
 * thereby updating their no of accesses, and then we insert a fourth entry and
 * expect that the entry that was least no of accesses is evicted. */
int test_cache_mru_lookup() {
  printf("running %s: ", __func__);
  ...
  /* At this point, the least frequently used entry is 8, 9 & 1, 7, but 8, 9 comes first in the order;
   * therefore, the next insert should evict it. */

  cache_insert(15, 255, in4);

  if (cache_lookup(8, 9, out) != -1) {
    printf("failed: the entry 8, 9 should have been evicted but it was not.\n");
    goto out;
  }
...
int test_cache_resize() {
  ...
  cache_create(3);
  bool success = false;
  int rc = cache_resize(20);
  if (rc == -1) {
    printf("failed: resizing cache should succeed but it failed.\n");
    goto out;
  }
  success = true;
